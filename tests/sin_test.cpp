...
void init_input_and_ref(float* a, float* ref, unsigned size)
{
    srand(2021);
...
int main(int argc, char* argv[])
{
    ...
    //Create the add kernel operating on the tensors
    status = synNodeCreate(graph, &in_tensor, &out_tensor, 1, 1, nullptr, 0, "sin_fwd_f32", "Sinus", nullptr, nullptr);
    if (status != synSuccess)
    {
    ...
    init_input_and_ref(input, ref, inZ * inW * inH * batch);
